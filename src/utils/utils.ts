...
const titleForRun = (run: Activity): string => {
  ...
  if (runDistance > 20 && runDistance < 40) {
    return `${run.name.includes('徒步') ? '徒步-' : ''}${RUN_TITLES.HALF_MARATHON_RUN_TITLE}`;
  }
  if (runDistance >= 40) {
    return `${run.name.includes('徒步') ? '徒步-' : ''}${RUN_TITLES.FULL_MARATHON_RUN_TITLE}`;
  }
  if (runHour >= 0 && runHour <= 10) {
    return `${run.name.includes('徒步') ? '清晨徒步' : RUN_TITLES.MORNING_RUN_TITLE}`;
  }
  if (runHour > 10 && runHour <= 14) {
    return `${run.name.includes('徒步') ? '午间徒步' : RUN_TITLES.MIDDAY_RUN_TITLE}`;
  }
  if (runHour > 14 && runHour <= 18) {
    return `${run.name.includes('徒步') ? '午后徒步' : RUN_TITLES.AFTERNOON_RUN_TITLE}`;
  }
  if (runHour > 18 && runHour <= 21) {
    return `${run.name.includes('徒步') ? '傍晚徒步' : RUN_TITLES.EVENING_RUN_TITLE}`;
  }
  return `${run.name.includes('徒步') ? '夜晚徒步' : RUN_TITLES.NIGHT_RUN_TITLE}`;
};
